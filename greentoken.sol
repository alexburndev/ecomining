...
/*
Green Token is part of the unified Ecomining system.
...
contract owned is TRC20Standard {
   ...
   // Token TRC20 GT = Token TRC20(TGc4179QHaJxEau8GN65SneJPH1FxTAwV4);
   ...
   uint8   public decimals    = 2;
    ...
    function starting() public onlyOwner returns(bool) {
      ...
      HolderData storage data = holders[owner];
        ...
        // На предмайнинг
        balanceOf[owner] = 10000000 * 10 ** uint256(decimals);
        balanceOf[addressSupportProject] = 1000000 * 10 ** uint256(decimals);
         // На Генезис
        balanceOf[thiscontract] =  totalSupply - balanceOf[owner]
       ...
       // emit Transfer(thiscontract, owner, balanceOf[owner] / 10 ** uint256(decimals));
    ...
    function reward_info(address _addressHolder) public view returns (uint256 Reward) {
       ...
       // 10 ** uint256(decimals);
        ...
        //стейкинг начинается от 1 монеты
...
contract Green_Token is Working {
  ...
  function compareAddress(address _tronAddress) public pure returns (string memory MSG, address Tron){
    ...
    string memory note,uint256 Total_Token_On_this_contract, uint256 Total_Users_on_this_contract,
    uint256 Token_Balance_On_My_Wallet, uint256 My_TokenBalance_On_this_Contract,
    uint256 Pending_Reward, uint256 Procent_Reward,
    uint256 LastProfitDate, bool InBlackList) {
        ...
        Token_Balance_On_My_Wallet = balanceOf[_addressHolder];
        Pending_Reward = reward_info (_addressHolder);
        Procent_Reward = P;
        Total_Token_On_this_contract = balanceOf[thiscontract];
       
        if (data.blackList != _addressHolder) 
            InBlackList = false ;
       
        My_TokenBalance_On_this_Contract = data.TokenOnAcc;
        Total_Users_on_this_contract = UserCount;
        LastProfitDate = data.lastDatetime;
    ...
    function Withdrow_All_Reward()  public returns (bool) {
        HolderData storage data = holders[msg.sender];
        uint256 Reward = reward_info (msg.sender);
        require(Reward >0);
        if (Reward > 0) {
            data.TokenOnAcc -= Reward;
            balanceOf[msg.sender] += Reward / 10 * 10 ** uint256(decimals);
            data.lastDatetime = now;
            emit Transfer (thiscontract, msg.sender, Reward / 10 * 10 ** uint256(decimals));
            return true;
        } else {
            return false;
        }
    ...
    function ReInvest_Reward() public returns (bool){
        HolderData storage data = holders[msg.sender];
        uint256 Reward = reward_info (msg.sender);
        require(Reward >0);
        if (Reward > 0) {
    ...
    //Вывод произвольной суммы. (стейкинг добавляется при этом к основной сумме)
    
    function withdraw_some_amount_from_Contract(uint256 _amount) public returns (bool){
        HolderData storage data = holders[msg.sender];
    ...
    // Вывод всех токенов из проекта
    
    function exit_from_project () public returns (bool){
        HolderData storage data = holders[msg.sender];
        require(data.TokenOnAcc > 0);
        uint256 Reward = reward_info (msg.sender);
        uint256 RealSum = (data.TokenOnAcc + Reward) / 10 * 10 ** uint256(decimals);
        if (RealSum > 0){
        data.TokenOnAcc = 0;
        data.lastDatetime = now;
        UserCount --;
